...
import com.yuno.schematicaneo.client.gui.GuiHelper;
...
class GuiSchematicMaterialsSlot extends GuiSlot {
    ...
    protected void drawSlot(int index, int x, int y, int par4, Tessellator tessellator, int mouseX, int mouseY) {
        ...
        final String amount = this.guiSchematicMaterials.isShowStacksInfo()
                ? wrappedItemStack.getFormattedAmount()
                : wrappedItemStack.getFormattedAmountSimple();
        final String amountrequired = this.guiSchematicMaterials.isShowStacksInfo()
                ? wrappedItemStack.getFormattedAmountRequired(strMaterialRequired, strMaterialAvailable)
                : wrappedItemStack.getFormattedAmountRequiredSimple(strMaterialRequired, strMaterialAvailable);
