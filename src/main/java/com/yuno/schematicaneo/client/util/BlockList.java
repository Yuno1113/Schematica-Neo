package com.yuno.schematicaneo.client.util;
...
public class BlockList {
    ...
    public static class WrappedItemStack {
        ...
        public String getFormattedAmountSimple() {
            final char color = this.placed < this.total ? 'c' : 'a';
        ...
        public String getFormattedAmountRequiredSimple(final String reqstr, final String avastr) {
            final int need = this.total - this.inventory - this.placed;
            if (this.inventory != -1 && need > 0) {
                return String.format("\u00a7c%s:%d", reqstr, need);
            } else {
                return String.format("\u00a7a%s", avastr);
            }
        }
