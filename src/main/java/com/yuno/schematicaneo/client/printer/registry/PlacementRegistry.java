...
public class PlacementRegistry {
    ...
    public void populatePlacementMaps() {
        ...
        addPlacementMapping(
            ...
            new PlacementData(PlacementData.PlacementType.PLAYER, -1, -1, 3, 2, 1, 0).setOffset(0x4, 0.0f, 1.0f)
                .setMaskMeta(0x3));
        // All slab subclasses need the click height to match the upper-half bit.
        // The block-specific registrations below still take precedence for their
        // material metadata and double-slab handling.
        addPlacementMapping(
            BlockSlab.class,
    ...
    public PlacementData getPlacementData(Block block, ItemStack itemStack) {
        if (isManaMetalSlab(block)) {
            return new PlacementData(PlacementData.PlacementType.BLOCK)
                .setOffset(0, 0.0f, 1.0f)
                .setManaMetalSlab();
        }
    ...
    private boolean isManaMetalSlab(Block block) {
        if (block == null) return false;
        String registryName = GameData.getBlockRegistry().getNameForObject(block);
        if (registryName == null || !registryName.startsWith("manametalmod:")) return false;
        return block instanceof BlockSlab
            || registryName.toLowerCase().contains("slab")
            || "manametalmod:BlockRenderCores3".equals(registryName);
    }
