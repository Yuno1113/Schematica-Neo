package com.yuno.schematicaneo.client.printer;
...
import com.yuno.schematicaneo.client.printer.registry.PlacementData;
import com.yuno.schematicaneo.client.printer.registry.PlacementRegistry;
import com.yuno.schematicaneo.client.util.BlockToItemStack;
...
public class SchematicPrinter {
    ...
    public boolean print() {
        if (this.sortByDistance) {
            return printByDistance();
        } else {
            return printLayerOrder();
        }
    }

    private boolean printLayerOrder() {
    ...
    private boolean printByDistance() {
        final EntityClientPlayerMP player = this.minecraft.thePlayer;
        final World world = this.minecraft.theWorld;
        ...
        final double px = ClientProxy.playerPosition.x;
        final double py = ClientProxy.playerPosition.y;
        final double pz = ClientProxy.playerPosition.z;
        ...
        final List<Vector3i> positions = new ArrayList<>();
        for (int y = minY; y < maxY; y++) {
            if (isRenderingLayer && y != renderingLayer) {
                continue;
            }
        ...
        positions.sort(Comparator.comparingDouble(v -> {
            final double wx = this.schematic.position.x + v.x;
            final double wy = this.schematic.position.y + v.y;
            final double wz = this.schematic.position.z + v.z;
            final double dx = wx + 0.5 - px;
            final double dy = wy + 0.5 - py;
            final double dz = wz + 0.5 - pz;
            return dx * dx + dy * dy + dz * dz;
        }));

        for (final Vector3i v : positions) {
            try {
                if (placeBlock(world, player, v.x, v.y, v.z)) {
                    player.inventory.currentItem = slot;
                    syncSneaking(player, isSneaking);
                    return true;
                }
    ...
    private boolean placeBlock(World world, EntityPlayer player, int x, int y, int z, Block block, int metadata,
        ...
        // Fallback: try placing against the block below the target position.
        // This helps re-place floating/dug-up blocks that have no solid side exposed.
        if (isSolid(world, x, y, z, ForgeDirection.DOWN)) {
            return placeBlock(world, player, x, y, z, ForgeDirection.DOWN, 0.0f, offsetY, 0.0f, extraClicks);
        }
