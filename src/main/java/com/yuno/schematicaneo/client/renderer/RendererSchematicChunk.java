...
import com.yuno.schematicaneo.client.renderer.shader.ShaderProgram;
...
public class RendererSchematicChunk {
    ...
    private static final ShaderProgram SHADER_ALPHA = new ShaderProgram("schematicaneo", null, "shaders/alpha.frag");
    ...
    private boolean hasBlocksChecked = false;
    ...
    private boolean checkHasBlocks() {
        int minX = (int) this.boundingBox.minX;
        int maxX = Math.min((int) this.boundingBox.maxX, this.schematic.getWidth());
        int minY = (int) this.boundingBox.minY;
        int maxY = Math.min((int) this.boundingBox.maxY, this.schematic.getHeight());
        int minZ = (int) this.boundingBox.minZ;
        int maxZ = Math.min((int) this.boundingBox.maxZ, this.schematic.getLength());
    ...
    public void updateRenderer() {
        if (this.needsUpdate) {
            ...
            if (!this.hasBlocksChecked) {
                this.hasBlocksChecked = true;
                this.hasBlocks = this.checkHasBlocks();
                if (!this.hasBlocks) {
                    return;
                }
            }
